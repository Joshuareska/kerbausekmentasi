...
class_names = {
    0: "Bonga (Price: ± 100 Juta – 200 Juta)",
    1: "Lotongboko (Price: ± 350 Juta – 400 Juta)",
    2: "Saleko",
    3: "Other Types"
}
...
def process_saleko(masked_image):
    ...
    if white_percentage > 90:
        price_estimation = "(± ≥ Rp 500 Juta)"
    else:
        price_estimation = "(± 350 Juta – 500 Juta)"
...
def predict_and_visualize(image_path):
    ...
    if image is None:
        st.error(f"Gambar tidak ditemukan di {image_path}")
        return
        ...
        if hasattr(result, 'boxes') and result.boxes is not None:
            for box in result.boxes:
                ...
                if class_id == 2:  # Saleko
                    ...
                    price_estimation = original_class_name.split("(")[-1][:-1]

                # Tambahkan bounding box
                color = (255, 0, 0)
                cv2.rectangle(output_image, (x1, y1), (x2, y2), color, 2)
                label = f"{original_class_name} ({confidence:.2f}) {price_estimation}"
                cv2.putText(output_image, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
                st.write(f"Class: {original_class_name}, Confidence: {confidence:.2f}, Price: {price_estimation}")
...
if uploaded_file is not None:
    image_path = f"upload/temp_{uploaded_file.name}" # Menyimpan gambar yang di uji pada folder upload
