...
st.write("Unggah gambar kerbau untuk mendeteksi jenis dan harga kerbau")
...
def predict_and_visualize(image_path):
    ...
    results_detection = model_detection.predict(source=resized_image, conf=0.80)
                ...
                if class_id == 2:  # Saleko
                    ...
                    for seg_result in results_segmentation:
                        if seg_result.masks is not None:
                            ...
                            valid_pixels = roi_grayscale[combined_mask_resized > 0]

                            # Simpan matriks grayscale valid ke Excel
                            grayscale_matrix = pd.DataFrame(roi_grayscale)
                            excel_path = "grayscale_values.xlsx"
                            grayscale_matrix.to_excel(excel_path, index=False, header=False)

                            # Klasifikasi warna grayscale
                            black_pixels = (valid_pixels <= 96).sum()
                            white_pixels = (valid_pixels > 96).sum()
                            ...
                            st.write(f"Grayscale values saved to {excel_path}")
