...
def visualize_and_classify_resized_grayscale(image_path, output_excel_path):
    ...
    model = YOLO('yolov8x-seg.pt')  # Gunakan model YOLO yang sudah dilatih untuk mendeteksi Kerbau

    # Baca gambar
    image = cv2.imread(image_path)
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

    # Deteksi dan segmentasi objek kerbau
    results = model.predict(image, conf=0.5)
    result = results[0]  # Mengambil hasil pertama jika ada lebih dari satu objek yang terdeteksi

    # Pastikan objek kerbau ada yang terdeteksi
    if result.masks is not None:
        # Dapatkan mask dari objek yang terdeteksi
        mask = result.masks.data[0].cpu().numpy()  # Ambil mask untuk objek kerbau pertama
        ...
        # Terapkan mask pada gambar untuk area kerbau
        masked_image = cv2.bitwise_and(image_rgb, image_rgb, mask=mask)

        # Konversi gambar hasil masking menjadi grayscale
        grayscale_image = cv2.cvtColor(masked_image, cv2.COLOR_RGB2GRAY)

        # Resize gambar ke ukuran yang di inginkan semisal 5 * 5
        resized_grayscale = cv2.resize(grayscale_image,
        (20, 20), interpolation=cv2.INTER_NEAREST)
        ...
        black_pixels = (valid_pixels <= 80).sum() #nilai 96 masih tergolong abu abu.90
        white_pixels = (valid_pixels > 80).sum() #nilai 96 masih tergolong abu abu.90
        ...
        print("Tidak ada objek kerbau yang terdeteksi.")

# Ganti 'path/to/image.jpg' dengan path gambar Anda dan 'output_excel_path' dengan lokasi file Excel yang akan disimpan
visualize_and_classify_resized_grayscale('kerbautes/xxxxxxxxxxxx.jpg', 'outputekstraksi/output_grayscalesalekob.xlsx')
